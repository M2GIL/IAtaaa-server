(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('iataaaServerApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-iataaaServerApp-error');
                var entityKey = httpResponse.headers('X-iataaaServerApp-params');
                if (errorHeader) {
