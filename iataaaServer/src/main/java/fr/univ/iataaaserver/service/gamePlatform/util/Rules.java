...
import fr.univ.iataaaserver.service.gamePlatform.Board;
...
public class Rules {
    ...
    private static final int CASE_NB_OF_LINE = 5;
    private static final int LINE_NB = 10;

    private static final int FIRST_POSITION_OF_LAST_LINE = CASE_NB_OF_LINE
        * (LINE_NB - 1);
    private static final int LAST_POSITION_OF_FIRST_LINE = CASE_NB_OF_LINE
        - 1;
    ...
    public static List<Board<Case>> getAvalaibleMoves(Board<Case> board, EnumPlayer p) {
        Case[] pieces = board.toArray();
        List<Integer> whitePieces;
        Case[] clonePieces;
        if (p == EnumPlayer.PLAYER_1) {
        ...
        if (p == EnumPlayer.PLAYER_1) {
            res = new ArrayList<>();
            for (Board<Case> ps : AVALAIBLE_MOVES) {
                res.add(new Board<>(reversePieces(ps.toArray())));
            }
        } else {
            res = AVALAIBLE_MOVES;
        }
...
// Elle devront être privé quand les tests ne serviront plus.
    ...
    private static void fillAvalaibleMovesForOnePiece(Case[] pieces, int srcPosition) {
        ...
        if (!isPossibleToJump && sizeMove == 1) { // S'il n'est pas possible de sauter une pièce.
            ...
            // Coté gauche du pion
                ...
                if (topLeftCornerPosition != -1) {
                    Case topleftcornerPiece = pieces[topLeftCornerPosition];
                    if (topleftcornerPiece == Case.EMPTY) { // Si je peux déplacer mon pion en haut à gaucher.
                        Case[] move = getMoveWhitePiece(pieces, srcPosition, topLeftCornerPosition);
                        AVALAIBLE_MOVES.add(new Board<>(move));
                    }
                }
                //------------------------------------------------------------------
                // Coté droit du pion
                ...
                if (topRightCornerPosition != -1) {
                    Case topRightcornerPiece = pieces[topRightCornerPosition];
                    if (topRightcornerPiece == Case.EMPTY) { // Si je peux déplacer mon pion en haut à droite.
                        Case[] move = getMoveWhitePiece(pieces, srcPosition, topRightCornerPosition);
                        AVALAIBLE_MOVES.add(new Board<>(move));
                    }
                }
                ...
                if (!positions.isEmpty()) {
                    ...
                    while (i < sizePositions && pieces[k] == Case.EMPTY) { // On ajoute toute les position de positions
                        ...
                        AVALAIBLE_MOVES.add(new Board<>(newPieces));
                        k = positions.get(i);
                        ++i;
                        if (i < sizePositions) {
                            k = positions.get(i);
    ...
    private static boolean sequenceJump(Case[] pieces, int srcPosition, final int size,
        ...
        if (size != 1 && !isPossibleToJump) {
            ...
            */
            ...
            if (size == sizeMove) {
                AVALAIBLE_MOVES.add(new Board<>(pieces));
    ...
    private static boolean addJumpedMoveToSequence(Case[] pieces, int srcPosition,
        ...
        if (jumpedPosition != -1 && tgtPosition != -1) {
            ...
            // Si on peut sauter un pion adverse dans le coin supérieur gauche
                isPossibleToJump = true;
                Case[] newPieces = getJumpBlackPiece(pieces, srcPosition, jumpedPosition, tgtPosition);
    ...
    private static Case[] getJumpBlackPiece(Case[] pieces, int srcPosition,
    ...
    private static Case[] getMoveWhitePiece(Case[] pieces, int srcPosition, int tgtPosition) {
        assert pieces[srcPosition] != Case.BLACK_PIECE;
        assert pieces[srcPosition] != Case.BLACK_QUEEN;
        assert pieces[srcPosition] != Case.EMPTY;
        assert pieces[tgtPosition] == Case.EMPTY;
    ...
    private static List<Integer> getWhitePieces(Case[] pieces) {
    ...
    private static List<Integer> getBlackPieces(Case[] pieces) {
        List<Integer> positions = new ArrayList<>(15);
        for (int i = 0; i < pieces.length; ++i) {
            if (pieces[i] == Case.BLACK_PIECE || pieces[i] == Case.BLACK_QUEEN) {
                positions.add(i);
            }
        }
        return positions;
    ...
    private static Case[] reversePieces(Case[] pieces) {
        Case[] reversePieces = new Case[PIECE_SIZE];
...
// Methode de debug
    ...
    private static void printAvalaibleMove() {
        System.out.println("--------------");
        System.out.println("DEBUG : PRINT AVALAIBLEMOVE");
        System.out.println("size = " + sizeMove);
        AVALAIBLE_MOVES.forEach((p) -> {
            Util.displayBoard(p.toArray());
        });
        System.out.println("-----------------------------");
    }
